...
OUT=HERE/"GEMINI_EXACT_STATE_ANNOTATION_V1_0.json"
...
Judge only the exact acting state below.
...
HISTORY:
"""+"\n".join(f"- {x}" for x in p["history"])+f"""

IMMEDIATELY PRECEDING LOCAL STEP:
{p['latest_step']}

Question: Is continuing or replanning the SAME CURRENT LOCAL TASK still a defensible highest-value next move for the GLOBAL GOAL?
