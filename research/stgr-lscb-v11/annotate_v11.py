...
PACKETS=json.loads((HERE/"EXACT_STATE_BLIND_PACKETS_V1_1.json").read_text(encoding="utf-8"))
OUT=HERE/"GEMINI_EXACT_STATE_ANNOTATION_V1_1.json"
def token(): return subprocess.check_output(["gcloud","auth","print-access-token"],text=True).strip()
def ask(p):
    hist="\n".join(f"- {x}" for x in p["history"])
    prompt=f"""You are an independent blind annotator. Judge only this exact acting state.
...
Is continuing or replanning the SAME CURRENT LOCAL TASK still a defensible highest-value next move for the GLOBAL GOAL?

Return JSON only:
{{"primary":"YES|NO|UNCERTAIN","confidence":1,"reason":"one concise sentence"}}

YES = same local task remains primary.
NO = task is complete or attention should move to another task/layer/bottleneck.
UNCERTAIN = exact state is insufficient."""
