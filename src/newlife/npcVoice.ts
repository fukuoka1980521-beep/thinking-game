...
/** Exported so `semantic/factsProjection.ts` (Phase 29 design) can cite the same raw fact string rather than hand-copying it — see that file's header comment. */
...
/** Exported for the same reason as `MENU_FACT` above — see `semantic/factsProjection.ts`. */
