...
const ALL_ITEMS: Record<string, ShopItem> = Object.fromEntries([...CAFE_MENU, ...YOHEI_GOODS].map((i) => [i.id, i]));

export function itemById(id: string): ShopItem | undefined {
  return ALL_ITEMS[id];
}

/** Only shop NPCs (currently Yohei, Miyoko) have a bounded menu at all -- Kamiya and Jin never
 *  sell anything, so their AI context simply never carries this field populated. */
