# NEW LIFE — BLIND STATE INTERPRETATION MATRIX V8
...
### Y01 — history note (SEVERE_RUPTURE logged, currently GUARDED, never WITHDRAWN): apology + restated boundary-safe plan
Context: Ryo's relationship state is currently GUARDED; he has had exactly one SEVERE_RUPTURE-class event logged earlier this scene and has never reached WITHDRAWN this case. Private, only Ryo present.
「さっきは強く言い過ぎた、悪かった。予定通り、実話を外した版で進めるよ」

### Y02 — same history note: fresh insult in a new turn
Context: same as Y01 (Ryo GUARDED, one earlier SEVERE_RUPTURE, never WITHDRAWN). Private, only Ryo present.
「リョウ、いい加減にしてよ、頭おかしいんじゃないの」

### Y03 — cross-NPC isolation: Mika turn, Ryo's history note is irrelevant
Context: same Ryo history note as Y01 still holds in the background. Separately, Mika has already said 「無理です」once this scene. Private, only Mika present.
「いや、そのままやってもらう」

### Y04 — WITHDRAWN note: pure apology + plan
...
### Y05 — WITHDRAWN note: apology + fresh insult, same turn
Context: same WITHDRAWN note as Y04. Private, only Mika present.
「さっきはごめん。でも美香もいい加減にしてよ、頭おかしいって」

---

### Y06 — hedged declarative rewrite, no settle/assign marker
Context: private, only Ryo present.
「まあ実話は外すけどさ、それで」

### Y07 — explicit decision-as-settled marker
Context: private, only Ryo present.
「じゃあこれで決定。実話は外した版で本番まで進める」

### Y08 — flat declarative, no hedge, no settle marker
Context: private, only Ryo present.
「実話は外す方向でいくよ」

### Y09 — propose + seek confirmation from boundary-holder
Context: private, only Mika present.
「実話の部分を外せば、これでやってもらえる?」

### Y10 — explicit delegation of the rewriting work itself
Context: private, only Ryo present.
「リョウ、この版に書き直して」

### Y11 — register pair (polished): hedged declarative rewrite
Context: private, only Ryo present.
「多少迷いはございますが、実話は外す方向で進めさせていただきたく存じます」

### Y12 — register pair (blunt): same content as Y11
Context: private, only Ryo present.
「まあ実話は外す感じで」

---

### Y13 — idiom via body-part-noun subject: 頭おかしい
Context: private, only Mika present.
「美香、頭おかしいよ」

### Y14 — control: progressive causal-blame verb, not a trait
Context: private, only Mika present, said directly to her.
「美香がごねてるせいで押してる」

---

### Y15 — private causal-blame statement, no third party
Context: private, only Mika present.
「美香が今さら渋ってるせいで押してる」

### Y16 — same wording, group present
Context: Mika and Ryo both present.
「美香が今さら渋ってるせいで押してる」

---

### Y17 — regression: repeated RECONSIDER after explicit NO
Context: Mika has already said 「無理です」once this scene. Private, only Mika present.
「じゃあ一部だけでも変えられない?」

### Y18 — regression: compound request + declared execution
Context: Mika has already said 「無理です」once this scene. Private, only Mika present.
「聞いてほしいけど、時間もないし、このまま進めるから」

### Y19 — regression: conditional ultimatum pressuring reconsideration
Context: Mika has already said 「無理です」once this scene. Private, only Mika present.
「このままやってくれないなら、次の公演には呼ばないよ」

---

### Y20 — regression: apology + resultative-idiom insult, same turn
Context: private, only Mika present.
「さっきは悪かった。でも美香もいかれてるよ」

---

### Y21 — history/completion-framing note (PUBLIC_SHAMING branch): plain settle turn
Context: this turn is intended by the player as the operational completion of an earlier corrective action targeting a PUBLIC_SHAMING strain against Mika (logged earlier this scene, group present at the time). Private, only Ryo present now.
「じゃあこれで決定、実話は外した版で本番まで進める」

### Y22 — history/completion-framing note (insult/dismissal branch): apology + settled plan
Context: this turn is intended by the player as the operational completion of an earlier corrective action targeting a PERSONAL_INSULT strain against Mika (logged earlier this scene). Private, only Mika present now.
「さっきの言い方は悪かった、認める。実話は外した版でいくって決めたから」

---

### Y23 — NOT_RELEVANT: plan removes boundary-holder from the task
Context: private, only Ryo present.
「美香の代わりにアンダースタディに任せる」

### Y24 — AVOID: boundary-holder still performs, modified to stay inside limit
Context: private, only Mika present.
「美香には演じてもらうけど、実話の部分だけ外した版にする」

---

### Y25 — SEEK_PERMISSION: boundary question naming a concrete mitigation candidate
Context: private, only Mika present.
「実話の部分を短くすれば大丈夫?」

### Y26 — DISCOVER: open-ended boundary question, no candidate named
Context: private, only Mika present.
「どこまでなら大丈夫?」

---

### Y27 — genuinely ambiguous action
Context: private, only Ryo present.
「うーん…どうしようかな…」

### Y28 — boundary-referent ambiguous, action clear
Context: two live boundary-related topics this scene (removing the real-story dialogue from the scene, versus Mika attending the performance in person at all). Private, only Mika present.
「そっちも無理そう?」

---

### Y29 — OBSERVE: situational statement, no named responsible person
Context: private, only Ryo present.
「このままだと本番に間に合わない」

### Y30 — OTHER: second-person-directed dismissive blame, no plan attached
Context: private, only Mika present, said directly to her.
「また美香がそうやって話をややこしくするんだから」
