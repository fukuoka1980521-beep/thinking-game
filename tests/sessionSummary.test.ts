...
const case001 = CASES[0]; // no uncertaintyChoiceId
...
describe("computeSessionSummary", () => {
  ...
  it("counts VERIFY and REJECT AI actions separately", () => {
    const logs = [
      makeLog({ aiIntervention: { message: "m", playerAction: "VERIFY", problemTypeSelected: "NONE", freeText: "" } }),
      makeLog({ aiIntervention: { message: "m", playerAction: "REJECT", problemTypeSelected: "NONE", freeText: "" } }),
      makeLog({ aiIntervention: { message: "m", playerAction: "ACCEPT", problemTypeSelected: "NONE", freeText: "" } }),
    ];
    const summary = computeSessionSummary(logs, "run-1");
  ...
  it("counts a case as 'chose uncertain' only when the case defines an uncertaintyChoiceId and it was picked", () => {
    const logs = [
      // case001 has no uncertaintyChoiceId — picking "d" should not count.
      makeLog({ caseId: case001.caseId, firstDecision: { choiceId: "d", confidence: 50, reason: "", factCheckAnswer: "fact", infoOptionsSelected: [] } }),
